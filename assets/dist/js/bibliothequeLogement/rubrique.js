url =
  "http://195.15.218.172/edlgateway/api/v1/rubric/all?start=0&limit=1&count=10";
...
function goWhereEdit(id) {
  $.cookie("id_bibliotheque_rubrique_edit", id);
  window.location.replace(
    "./../../bibliothequeLogement/rubrique/modifier.html"
  );
}
  ...
  $.ajax({
    ...
    success: function () {
      ...
      $.toaster({
        priority: "success",
        title: "success",
        message: "Rubrique crée avec succès.",
      });
      setInterval(
        () =>
          window.location.replace(
            "./../../bibliothequeLogement/rubrique/index.html"
          ),
        2000
      );
    },
...
function getSingleRubrique() {
  let rubriqueId = $.cookie("id_bibliotheque_rubrique_edit");
  ...
  $.ajax({
    type: "GET",
    url:
      "http://195.15.218.172/edlgateway/api/v1/single/rubric/?ID=" + rubriqueId,
    success: (response) => {
      $("#waiters").css("display", "none");
      $("#form-content").css("display", "block");

      let rubrique = response.results;

      $("#nom").val(rubrique.nom).trigger("change");
      $("#description").val(rubrique.description).trigger("change");
      rubrique.status === "on"
...
function editRubrique() {
  ...
  data["id"] = $.cookie("id_bibliotheque_rubrique_edit");

  $.ajax({
    type: "PUT",
    crossDomain: true,
    dataType: "json",
    url: "http://195.15.218.172/edlgateway/api/v1/Rubric/modify",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
      "content-Type": "application/json",
      "Access-Control-Allow-Origin": "*",

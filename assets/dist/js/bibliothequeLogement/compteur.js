url =
  "http://195.15.218.172/edlgateway/api/v1/compteurs/all?start=0&limit=1&count=10";
...
function goWhereEdit(id) {
  ...
  window.location.replace(
    "./../../bibliothequeLogement/compteur/modifier.html"
  );
}
  ...
  $.ajax({
    ...
    success: function () {
      ...
      $.toaster({
        priority: "success",
        title: "success",
        message: "Compteur crée avec succès...Redirection en cours...",
      });
      setInterval(
        () =>
          window.location.replace(
            "./../../bibliothequeLogement/compteur/index.html"
          ),
        2000
      );
    },
...
function getSingleCompteur() {
  ...
  $.ajax({
    type: "GET",
    url:
      "http://195.15.218.172/edlgateway/api/v1/single/compteurs/?ID=" +
      rubriqueId,
    success: (response) => {
      $("#waiters").css("display", "none");
      ...
      $("#nom").val(compteur.nom);
      $("#description").val(compteur.description);
      $("#numero_d_ordre").val(compteur.numero_d_ordre);
      if (compteur.status === "on") $("#status").prop("checked", true);
    },
    error: (response) => {},
  });
}

function editCompteur() {
  $("#go").html("Enregistrement en cours...");
  ...
  $.ajax({
    ...
    crossDomain: true,
    dataType: "json",
    url: "http://195.15.218.172/edlgateway/api/v1/compteurs/modify",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
      "content-Type": "application/json",
      "Access-Control-Allow-Origin": "*",
    ...
    success: function () {
      ...
      $.toaster({
        priority: "success",
        title: "success",
        message: "Compteur crée avec succès.",
      });
      window.location.replace(
        "./../../bibliothequeLogement/compteur/index.html"
      );

url =
  "http://195.15.218.172/edlgateway/api/v1/piece/all?start=0&limit=1&count=10";
...
function goWhereEdit(id) {
  $.cookie("id_bibliotheque_piece_edit", id);
  window.location.replace("./../../bibliothequeLogement/piece/modifier.html");
}

function addPiece(rubriques) {
  ...
  if (
    ...
    !$("#description").val() ||
    !$("#status") ||
    !Object.keys(rubriques).length === 0
  ) {
  ...
  $.ajax({
    ...
    success: function () {
      ...
      $.toaster({
        priority: "success",
        title: "success",
        message: "Piece crée avec succès...Redirection en cours...",
      });
      setInterval(
        () =>
          window.location.replace(
            "./../../bibliothequeLogement/piece/index.html"
          ),
        2000
      );
    },
...
async function getSinglePiece() {
  let rubriqueId = $.cookie("id_bibliotheque_piece_edit");
  ...
  return $.ajax({
    type: "GET",
    url:
      "http://195.15.218.172/edlgateway/api/v1/single/piece/?ID=" + rubriqueId,
  });
}

function editPiece(rubriques) {
  $("#go").html("Enregistrement en cours...");
  ...
  data["id"] = $.cookie("id_bibliotheque_piece_edit");

  $.ajax({
    type: "PUT",
    crossDomain: true,
    dataType: "json",
    url: "http://195.15.218.172/edlgateway/api/v1/piece/modify",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
      "content-Type": "application/json",
      "Access-Control-Allow-Origin": "*",
    ...
    success: function () {
      $.toaster({
        priority: "success",
        title: "success",
        message: "Piece modifier avec succès...Redirection en cours...",
      });
      setInterval(
        () =>
          window.location.replace(
...
async function loadRubrique() {
  return $.ajax({
    type: "GET",
    crossDomain: true,
    dataType: "json",
    url: "http://195.15.218.172/edlgateway/api/v1/rubric/all?start=0&limit=1&count=10",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
      "content-Type": "application/json",
      "Access-Control-Allow-Origin": "*",

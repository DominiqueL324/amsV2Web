url =
  "http://195.15.218.172/edlgateway/api/v1/voie/all?start=0&limit=1&count=10";
...
function goWhereEdit(id) {
  $.cookie("id_bibliotheque_voie_edit", id);
  window.location.replace("./../../bibliothequeLogement/voie/modifier.html");
}
...
function addVoie() {
  ...
  $.ajax({
    ...
    crossDomain: true,
    dataType: "json",
    url: "http://195.15.218.172/edlgateway/api/v1/voie/add",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
    ...
    success: function () {
      ...
      $.toaster({
        priority: "success",
        title: "success",
        message: "Voie crée avec succès...Redirection en cours...",
      });
      setInterval(
        () =>
          window.location.replace(
            "./../../bibliothequeLogement/voie/index.html"
          ),
        2000
      );
    },
...
function getSingleVoie() {
  let voieId = $.cookie("id_bibliotheque_voie_edit");

  $("#form-content").css("display", "none");

  $.ajax({
    type: "GET",
    url: "http://195.15.218.172/edlgateway/api/v1/single/voie/?ID=" + voieId,
    success: (response) => {
      $("#waiters").css("display", "none");
      $("#form-content").css("display", "block");

      let voie = response.results;

      console.log(voie);

      $("#libelle").val(voie.nom).trigger("change");
      $("#numero_d_ordre").val(voie.numero_d_ordre).trigger("change");
...
function editVoie() {
  ...
  data["numero_d_ordre"] = $("#numero_d_ordre").val();
  data["status"] = $("#status").is(":checked") ? "on" : "off";
  data["id"] = $.cookie("id_bibliotheque_voie_edit");

  $.ajax({
    type: "PUT",
    crossDomain: true,
    dataType: "json",
    url: "http://195.15.218.172/edlgateway/api/v1/voie/modify",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
      "content-Type": "application/json",
      "Access-Control-Allow-Origin": "*",
    ...
    success: function () {
      ...
      $.toaster({
        priority: "success",
        title: "success",
        message: "Voie modifier avec succès.",
      });
      window.location.href = "./../../bibliothequeLogement/voie/index.html";
    },
    error: function () {
      $.toaster({
        priority: "danger",

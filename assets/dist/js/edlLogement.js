...
function addEDLLogement() {
  ...
  if ($.cookie("signataire_list")) {
    rdv_edl_signataire = new Map(JSON.parse($.cookie("signataire_list")));
  } else {
    rdv_edl_signataire = new Map();
  }

  if ($.cookie("intervenant")) {
    intervenant = JSON.parse($.cookie("intervenant"));
  }
  ...
  data["logement"] = $.cookie("id_logement_edl");
  ...
  data["created_by"] = $.cookie("id_user_logged");
  ...
  $.ajax({
    ...
    success: () => {
      ...
      if ($.cookie("rdv_edl_list")) $.removeCookie("rdv_edl_list");
      if ($.cookie("signataire_list")) $.removeCookie("signataire_list");
      if ($.cookie("intervenant")) $.removeCookie("intervenant");
...
function renderRdvList() {
  ...
  if ($.cookie("rdv_edl_list")) {
    edl_list = new Map(JSON.parse($.cookie("rdv_edl_list")));
  }
...
function addRdv() {
  ...
  signataire_list = new Map(JSON.parse($.cookie("signataire_list")));
...
function deleteRdv(key) {
  ...
  edl_list.delete(key);
  $.cookie("rdv_edl_list", JSON.stringify(Array.from(edl_list.entries())));

url = "http://195.15.228.250/amscommentaire/import_app/commentaires/?nature=cles&page=1";
var page_actuelle = 0;
var total_page = 0;
var suivant=false;
var precedent=false;
...
function getAllCle() {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      response["results"].slice(start, end).forEach((elt) => {
        let className = "";
        let value = "";
        let cle = "";
...
function paginate(direction) {
  ...
  if (direction === "next") {
    if(suivant==true){
     url = "http://195.15.228.250/amscommentaire/import_app/commentaires/?nature=cles&page="+(page_actuelle+1).toString();
     getAllCle();
    }else{
     alert('Dernière page')
    }
...
async function openEditCommentModal(id) {
  ...
  await $.ajax({
    ...
    $.ajax({
      type: "GET",
      url: "http://195.15.218.172/edlgateway/api/v1/clefs/all?start=0&limit=10&count=100",
      headers: {
        Authorization: "Bearer " + localStorage.getItem("token"),
      },
      success: function (response) {
...
function addComment() {
  ...
  if (
    ...
    $.toaster({
      priority: "danger",
      title: "danger",
      message: "Remplicez tous les champs",
    });

    return;
  }
  data["key"] = $("#key_add").val();
  data["nature"] = $("#nature_add").val();
  data["status"] = $("#status_add").is(":checked") ? "on" : "off";
  data["commentaire"] = $("#commentaire_add").val();
  data["type"] = $("#type_add").val();
...
function editComment() {
  ...
  data["type"] = $("#type").val();
  data["id"] = $("#id").val();
  data["code"] = cles_['code'];
  data["numero_ordre"] = cles_['numero_ordre'];
...
$("#search--input").on("input", function (e) {
  ...
  if (value.length > 2) {
    ...
    $.ajax({
      type: "GET",
      url: "http://195.15.228.250/amscommentaire/import_app/commentaires/filter/?nature=cles&to_find="+value.toString(),
      headers: {
        Authorization: "Bearer " + localStorage.getItem("token"),
      },
      success: function(response){

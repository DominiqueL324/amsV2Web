url = "http://195.15.228.250/amscommentaire/import_app/commentaires/?nature=rubrique&page=1";
...
function paginate(direction) {
  if (direction === "next") {
    if(suivant==true){
     url = "http://195.15.228.250/amscommentaire/import_app/commentaires/?nature=rubrique&page="+(page_actuelle+1).toString();
     getAllRubrique();
    }else{
     alert('Dernière page')
    }
...
async function openEditCommentModal(id) {
  ...
  await $.ajax({
    ...
    $.ajax({
      type: "GET",
      url: "http://195.15.218.172/edlgateway/api/v1/rubric/all?start=0&limit=10&count=100",
      headers: {
        Authorization: "Bearer " + localStorage.getItem("token"),
      },
      success: function (response) {
...
function editComment() {
  ...
  data["code"] = rubrique_['code'];
  data["numero_ordre"] = rubrique_['numero_ordre'];
...
$("#search--input").on("input", function (e) {
  ...
  if (value.length > 2) {
    $.ajax({
      type: "GET",
      url: "http://195.15.228.250/amscommentaire/import_app/commentaires/filter/?nature=rubrique&to_find="+value.toString(),
      headers: {
        Authorization: "Bearer " + localStorage.getItem("token"),
      },
      success: function(response){
